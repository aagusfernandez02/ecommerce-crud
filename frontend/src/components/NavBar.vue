...
<template>
  <v-navigation-drawer
    v-model="shoppingCartStore.navigationDrawerOpen"
    temporary
  ...
  <v-app-bar :elevation="0" :color="'var(--app-bar-color)'" class="appbar">
    ...
    <template v-slot:append>
      <v-menu transition="slide-y-transition" v-model="menuOpened">
        ...
        <template v-slot:activator="{ props }" v-else>
          <v-btn
            :append-icon="menuOpened ? 'mdi-chevron-up' : 'mdi-chevron-down'"
            v-bind="props"
            size="x-large"
          >
        ...
        <v-list>
          <v-list-item
            ...
            class="mouse-hover-click"
          >
            <v-list-item-title @click="handleAdminPanel"
              ><v-icon class="mr-2">mdi-security</v-icon>Admin
              Panel</v-list-item-title
            >
          </v-list-item>
          <v-list-item class="mouse-hover-click border-t-md">
            <v-list-item-title @click="handleLogout"
              ><v-icon class="mr-2">mdi-logout</v-icon>Sign
      ...
      <v-btn
        ...
        stacked
        @click="shoppingCartStore.handleToggleShoppingCartDrawer"
      >
